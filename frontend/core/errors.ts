...
export class ApiError extends CustomError {
  ...
  public constructor(
    ...
    err(`API Error: ${method} ${endpoint} [${status}] - ${message}`, responseData);
  }
}
...
export class NotFoundError extends CustomError {
  ...
  public constructor(message: string = 'Resource not found', resource?: string) {
    super(message, 404);
 ...
 */
export function requireValue<T>(value: T | null | undefined, label: string): asserts value is T {
 ...
 */
export function invalidParameters(message: string = 'Invalid parameters'): never {
 ...
 * @throws {CustomError} Always throws
 */
export function formatError(message: string, status = 400, error?: unknown): never {
 ...
 */
export function handleApiError(error: unknown, endpoint: string, method: string = 'GET'): never {
